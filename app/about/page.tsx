...
export default function AboutPage() {
  return (
    <>
      <section className="border-b border-black/10 px-5 py-12 md:px-8 md:py-20">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          <div>
            <p className="mb-5 text-sm font-semibold uppercase tracking-[0.12em] text-accent">Founder story</p>
            <h1 className="font-heading text-6xl font-semibold uppercase leading-[0.88] md:text-8xl">
              I am learning this in public.
            </h1>
            <p className="mt-4 text-sm text-black/70">Founder: a manager learning AI-assisted building in real time and documenting practical, testable lessons.</p>
          </div>
          <p className="max-w-2xl text-xl leading-9 text-black/80 md:text-2xl md:leading-10">
      ...
      >
        <div className="grid gap-4 text-lg font-semibold md:grid-cols-2">
          {["Marketing managers", "Operations managers", "Project managers", "Program managers", "Team leads", "Consultants", "Functional leaders", "Curious generalists"].map((person) => (
            <div key={person} className="border-t-2 border-black/15 py-4">
              {person}
            </div>
          ))}
        </div>
